...
import {
    request
} from "../steps.config.js";
import {
    resetDatabase
} from "../utils/reset-db.js";
...
defineFeature(feature, (test) => {
    ...
    test("User views their enrolled classes", ({
        given,
        and,
        when,
        then
        ...
        // First enrollment
        and(/^the user is enrolled in class "(.*)" with role "(.*)"$/, async (className, role) => {
        ...
        when("the user requests their class list", async () => {
            context.response = await request
                .get(`/api/classes/user/classes?userId=${context.user.id}`);
        });
        ...
        // First assertion
        and(/^the response should include "(.*)" with role "(.*)"$/, (className, role) => {
    ...
    test("Missing userId parameter returns error", ({
        when,
        then,
        and
    }) => {
    ...
    test("User views class list HTML page", ({
        given,
        and,
        when,
        then
        ...
        when("the user requests the HTML class list page", async () => {
            context.response = await request
                .get(`/api/classes/my-classes?userId=${context.user.id}`);
        });
