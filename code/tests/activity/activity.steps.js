...
defineFeature(feature, (test) => {
  ...
  test("Create a TA activity punch", ({ given, when, then, and }) => {
    given(/^a logged-in TA "(.*)" with email "(.*)" exists$/, async (name, email) => {
  ...
  test("Create an all activity punch as a student", ({ given, when, then, and }) => {
    given(/^a logged-in student "(.*)" with email "(.*)" exists$/, async (name, email) => {
      // Make sure a student exists
      context.user = await prisma.user.create({
        data: { email: email, name: name, isProf: false },
  ...
  test("Get an activity punch by ID", ({ given, when, then, and }) => {
    ...
    and(/^an activity punch for "Studying" exists and belongs to "Bob Student"$/, async () => {
        const activityData = {
            classId: context.klass.id,
            categoryId: context.category.id,
            userId: context.user.id,
    ...
    when(/^the student attempts to get an activity punch with ID$/, async () => {
        context.response = await request
  ...
  test("Get all activities from a student", ({ given, when, then, and }) => {
    ...
    and(/^an activity punch for "Lecture" exists and belongs to "Bob Student"$/, async () => {
        const activityData = {
            classId: context.klass.id,
  ...
  test("Update an activity punch", ({ given, when, then, and }) => {
    ...
    when(/^the student tries to update the category to "Lecture"$/, async () => {
        const activityData = {
            classId: context.klass.id,
            categoryId: context.category2.id,
            startTime: "2025-01-01T10:00:00Z",
