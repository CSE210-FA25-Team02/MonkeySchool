...
export class SubMenu {
  ...
  init() {
    ...
    if (!this.container) {
      console.error(`SubMenu: Container with id "${this.containerId}" not found`);
      return;
    }
  ...
  open(navItem, navElement) {
    ...
    if (this.isOpen && this.currentNavItem && this.currentNavItem.path !== navItem.path) {
  ...
  _addClickOutsideListener() {
    ...
    this.clickOutsideHandler = (event) => {
      ...
      if (this.currentNavElement && this.currentNavElement.contains(event.target)) {
        return;
      }
